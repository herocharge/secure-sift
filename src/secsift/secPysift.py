...
def secFindScaleSpaceExtrema(gaussian_images, dog_images, num_intervals, sigma, image_border_width, contrast_threshold=0.04):
    """
            ...
            for i in range(image_border_width, first_image.shape[0] - image_border_width):
                for j in range(image_border_width, first_image.shape[1] - image_border_width):
                    ...
                    if isPixelAnExtremum(first_image[i-1:i+2, j-1:j+2], second_image[i-1:i+2, j-1:j+2], third_image[i-1:i+2, j-1:j+2], threshold):
                        print("Extremum found")
                        localization_result = localizeExtremumViaQuadraticFit(i, j, image_index + 1, octave_index, num_intervals, dog_images_in_octave, sigma, contrast_threshold, image_border_width)
                        print("localization_result: ", localization_result)
                        if localization_result is not None:
                            keypoint, localized_image_index = localization_result
                            keypoints_with_orientations = computeKeypointsWithOrientations(keypoint, octave_index, gaussian_images[octave_index][localized_image_index])
                            # keypoints_with_orientations = [keypoint]
...
def isPixelAnExtremum(first_subimage, second_subimage, third_subimage, threshold):
    ...
    print("first_subimage shape: ", first_subimage.shape)
...
def localizeExtremumViaQuadraticFit(i, j, image_index, octave_index, num_intervals, dog_images_in_octave, sigma, contrast_threshold, image_border_width, eigenvalue_ratio=10, num_attempts_until_convergence=5):
    ...
    for attempt_index in range(num_attempts_until_convergence):
        ...
        ltsq_val = secLTSQ(hessian, gradient, rcond=None)[0]
    ...
    if attempt_index >= num_attempts_until_convergence - 1:
        ...
        return None
    functionValueAtUpdatedExtremum = pixel_cube[1, 1, 1] + 0.5 * secDot1D(gradient, extremum_update)
    # if abs(functionValueAtUpdatedExtremum) * num_intervals >= contrast_threshold:
    if True:
        xy_hessian = hessian[:2, :2]
        xy_hessian_trace = secTrace(xy_hessian)
        xy_hessian_det = secDet(xy_hessian)
